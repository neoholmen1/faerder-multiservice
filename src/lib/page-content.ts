...
export async function savePageHero(
  ...
  if (!supabase) return { error: "Database er ikke konfigurert." };
...
export async function savePageSections(
  ...
  const { error } = await supabase
    .from("page_sections")
    .upsert(rows, { onConflict: "site_id,page_slug,section_key,field_key,sort_order" });
  return { error: error?.message ?? null };
}

export function buildSectionMap(sections: PageSection[]): Record<string, Record<string, string>> {
